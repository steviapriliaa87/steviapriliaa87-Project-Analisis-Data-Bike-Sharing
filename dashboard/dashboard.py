...
hourly_filtered = hour_df[hour_df['date'].dt.year.isin(selected_year)]
hourly_rentals = hourly_filtered.groupby("hour")["total_rentals"].mean().reset_index()
fig = px.line(hourly_rentals, x='hour', y='total_rentals', labels={'hour': 'Jam', 'total_rentals': 'Jumlah Penyewaan'}, markers=True)
st.plotly_chart(fig)

# 2. Penyewaan Berdasarkan Bulan
df_filtered["month"] = pd.Categorical(df_filtered["month"], categories=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], ordered=True)
...
df_filtered["one_of_week"] = pd.Categorical(df_filtered["one_of_week"], categories=["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"], ordered=True)
...
hourly_filtered["weather_condition"] = pd.Categorical(hourly_filtered["weather_condition"], categories=["clear", "misty", "light rain/light snow", "bad weather"], ordered=True)
avg_rentals_by_weather = hourly_filtered.groupby('weather_condition')['total_rentals'].mean().reset_index()
...
selected_compare_years = st.multiselect("Pilih Tahun untuk Dibandingkan", df_filtered['date'].dt.year.unique(), default=[2011, 2012])

monthly_trend = df_filtered.groupby(["date", "month"], observed=True)["total_rentals"].sum().reset_index()
monthly_trend["month"] = pd.Categorical(monthly_trend["month"], categories=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], ordered=True)
monthly_trend = monthly_trend[monthly_trend["date"].dt.year.isin(selected_compare_years)]

fig = px.line(monthly_trend, x="month", y="total_rentals", color=monthly_trend["date"].dt.year.astype(str), 
              markers=True, labels={"month": "Bulan", "total_rentals": "Total Penyewaan", "date": "Tahun"}, 
