...
selected_renter_type = st.sidebar.radio("Pilih Jenis Penyewa", ["Semua", "Registered", "Casual"], index=0)
...
if selected_renter_type == "Registered":
    df_filtered["total_rentals"] = df_filtered["registered_rentals"]
elif selected_renter_type == "Casual":
...
hourly_rentals = hour_df[hour_df['weather_condition'].isin(selected_weather)].groupby("hour")["total_rentals"].mean().reset_index()
...
avg_rentals_by_weather = hour_df[hour_df['weather_condition'].isin(selected_weather)].groupby('weather_condition', observed=True)['total_rentals'].mean().reset_index()
...
fig = px.line(monthly_trend, x="month", y="total_rentals", color="year", markers=True, labels={"month": "Bulan", "total_rentals": "Total Penyewaan", "year": "Tahun"}, title="Perbandingan Tren Penyewaan Sepeda")
st.plotly_chart(fig)
